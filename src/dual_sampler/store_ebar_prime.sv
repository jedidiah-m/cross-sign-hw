...
logic [18*Z_BIT_WIDTH-1:0] d_buffer, q_buffer;
...
if(finished)begin//update
   ...
   if((valid_serial) && (!valid_parallel))begin
      d_buffer = {q_buffer,data_parallel[3*Z_BIT_WIDTH-1 -: Z_BIT_WIDTH]};
   ...
   if((!valid_serial) && (!valid_parallel) && (blast_output) && (!sample_type))begin//change in d_write(needs_editing)
      d_buffer = q_buffer;
      d_buffer_count = q_buffer_count;
...
assign finished = (finish_process)&(sample_type);//changed finished signal
...
always_comb begin
   ...
   if(d_write) begin
      ...
      if(q_sub_counter == COL_C)begin
         place_memory[16*Z_BIT_WIDTH-1:(16-PART)*Z_BIT_WIDTH] = q_buffer;
         place_memory[(16-PART)*Z_BIT_WIDTH-1:0] = 0;
      end
   end
end
